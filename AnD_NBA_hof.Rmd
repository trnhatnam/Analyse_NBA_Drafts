...
```{r}
data_old_player_career$hall_of_fame <- ifelse(data_old_player_career$hall_of_fame == 1, "Yes", "No")
data_old_player_career$hall_of_fame <- as.factor(data_old_player_career$hall_of_fame)
data_old_player_career$indiv_awards <- as.factor(data_old_player_career$indiv_awards)

# Trouver les colonnes de type entier
int_columns <- sapply(data_old_player_career, is.integer)

# Convertir les colonnes de type entier en numérique
data_old_player_career[int_columns] <- lapply(data_old_player_career[int_columns], as.numeric)


head(data_old_player_career)
str(data_old_player_career)
table(data_old_player_career$hall_of_fame)
Hall_of_Fame_In = table(data_old_player_career$hall_of_fame)[2]/dim(data_old_player_career)[1]
Hall_of_Fame_In
Hall_of_Fame_Out = table(data_old_player_career$hall_of_fame)[1]/dim(data_old_player_career)[1]
Hall_of_Fame_Out
```
...
```{r}
set.seed(1)
n <- nrow(data_old_player_career)
p <- ncol(data_old_player_career)-1
test.ratio <- .2 # ratio of test/train samples
n.test <- round(n*test.ratio)
n.test
tr <- sample(1:n,n.test)
data.test <- data_old_player_career[tr,]
data.train <- data_old_player_career[-tr,]
```
```{r}
...
taux_Y = table(data.train.balanced$hall_of_fame)[2]/dim(data.train.balanced)[1]
taux_Y
taux_N = table(data.train.balanced$hall_of_fame)[1]/dim(data.train.balanced)[1]
taux_N
```

...
On va déterminer quelles variables sont pertinents pour la mise en place de nos algorithmes
...
joueurs <- c('Allen Iverson', 'Dominique Wilkins', 'Kendrick Perkins', 'David West')
...
Après avoir exécuter la vérification de l'impacte de l'AIC, on regarde désormais les erreurs de prévisions de ce nouveau modèle 
...
```
```{r}
acc_LDA = sum(table(data.train.balanced$hall_of_fame,pred.lda)[1,1],table(data.train.balanced$hall_of_fame,pred.lda)[2,2])/dim(data.train.balanced)[1]
acc_LDA
```
On regarde la répartition des valeurs 'Yes' et 'No' :
...
```{r}
arbre_2 = rpart(hall_of_fame~.,data.test,control =rpart.control(minsplit = 5,cp = 0.037))
...
```
Il va falloir détailler et peut etre pour finir, de mieux sélectionner d'après nos connaissances les bonnes colonnes pour sélectionner un modèle.
